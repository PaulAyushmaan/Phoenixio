...
def transcribe_audio_chunks(audio_chunk_paths, audio_chunk_duration=20):
    ...
    try:
        ...
        total_duration_sec = sum(AudioSegment.from_file(p).duration_seconds for p in audio_chunk_paths)

        for idx, chunk_path in enumerate(audio_chunk_paths):
            logger.info(f"Transcribing chunk {idx + 1}/{len(audio_chunk_paths)}: {chunk_path}")

            result = asr_pipe(chunk_path)

            # Calculate chunk start/end boundaries
            chunk_start_time = idx * audio_chunk_duration
            chunk_end_time = min((idx + 1) * audio_chunk_duration, total_duration_sec)

            for segment in result.get('chunks', []):
                seg_start, seg_end = segment.get('timestamp', (None, None))

                if seg_start is not None and seg_end is not None:
                    adjusted_start = min(seg_start + chunk_start_time, total_duration_sec)
                    adjusted_end = min(seg_end + chunk_start_time, total_duration_sec)
                else:
